<template>
  <header class="promo event" id="uniqintro" data-hash="0" data-era="1">
    <div class="promo-preview invisible"></div>
  </header>
  ...
  <header class="nav">
    ...
    <div class="select_wrap">
      <div class="select">
        <select @change="scrollTo" v-model="activeCategory">
          <option value="" disabled selected>Категория</option>
          <option
            v-for="category in eventCategory"
            :key="category.id"
            :value="'#uniq' + category.events[0].id"
          >
            {{ category.title }}
          </option>
        </select>
  ...
  <div class="wrap">
    ...
    >
      <div
        class="event"
        :class="{
          important: event.type == 'history' || event.type == 'event',
